...
export function ReimbursementCertificate({
  ...
  const handleImprimir = () => {
    const contenido = document.getElementById('documento-imprimible');
    if (!contenido) return;

    // Clonar el contenido y remover los elementos no imprimibles (botones de edición, inputs)
    const clon = contenido.cloneNode(true) as HTMLElement;
    clon.querySelectorAll('.print-hide, button').forEach(el => el.remove());

    // Convertir los inputs de fecha y hora a texto plano para impresión
    clon.querySelectorAll('input').forEach(input => {
      const span = document.createElement('span');
      span.textContent = (input as HTMLInputElement).value;
      span.className = 'font-bold text-slate-800 text-xs text-center min-w-[60px] inline-block';
      if (input.parentNode) {
    ...
    if (!ventanaImpresion) {
      window.print(); // Fallback si el navegador bloquea popups
      return;
    }
    ...
    ventanaImpresion.document.write(`
      ...
      <html lang="es">
        <head>
          <title>Certificado de Asistencia y Tratamiento Kinésico - Kiromov</title>
          <meta charset="utf-8" />
          <style>
            @page {
        ...
        <body>
          <div style="width: 100%; max-width: 750px; margin: 0 auto; background: white; padding: 20px;">
            ${clon.innerHTML}
          </div>
          <script>
            window.onload = () => {
      ...
      <div className="relative w-full max-w-4xl max-h-[94vh] flex flex-col bg-slate-100 rounded-2xl shadow-2xl border border-slate-300 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-8">
          ...
          {loading ? (
            ...
            <div id="documento-imprimible" className="bg-white p-8 sm:p-12 max-w-3xl mx-auto shadow-sm border border-slate-200 font-serif text-slate-900">
              ...
              <div className="space-y-4 text-sm leading-relaxed text-slate-800 text-justify">
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 font-sans text-xs space-y-2 my-3">
                  ...
                  <div className="flex gap-1.5 items-center">
                    ...
                    <input
                      ...
                      className="w-full border-b border-dashed border-slate-300 p-0 bg-transparent text-xs text-slate-900 focus:ring-0 focus:border-blue-500 outline-none"
                    />
                  </div>
              ...
              <div className="my-6">
                <table className="w-full text-left text-[11px] sm:text-xs border border-slate-300 font-sans">
                  <thead className="bg-slate-100 border-b border-slate-300 font-semibold text-slate-800">
                    <tr>
                      ...
                      <th className="p-2 print-hide w-8 text-center"></th>
                    </tr>
                  </thead>
                      ...
                      <tr key={s.id} className="hover:bg-slate-50/50">
                        ...
                        <td className="p-2 border-r border-slate-200">
                          <input
                            ...
                            className="w-full border-none p-0 bg-transparent text-[11px] sm:text-xs text-slate-900 focus:ring-1 focus:ring-blue-500 rounded"
                          />
                        </td>
                ...
                <div className="flex justify-end mt-2 print-hide">
                  <button
                    type="button"
              ...
              <div className="my-6 space-y-2 text-xs font-sans bg-slate-50 p-4 rounded-xl border border-slate-200">
                ...
                <div className="flex flex-col sm:flex-row gap-4 mb-2">
                  <div className="flex-1 flex gap-2 items-center">
                    ...
                    <input
                      ...
                      className="w-24 border-b border-slate-300 p-0 bg-transparent font-bold focus:ring-0 focus:border-blue-500 outline-none text-center"
                    />
                  </div>
                  <div className="flex-1 flex gap-2 items-center">
